...
namespace Swimbait.ConsoleApp
{
    public class Program
    {
        public static void Main(string[] args)
        {
            ...
            var environmentService = new EnvironmentService(swimbaitConfig);

            
            var realMusicCastSpeakerIp = swimbaitConfig["RelayHost"];
            ...
            foreach(var line in activity)
            {
                ...
                if (log.Method == "GET")
                {
                    ...
                    var yamahaResponse = UriService.GetResponse(IPAddress.Parse(realMusicCastSpeakerIp), log.YamahaPort, log.PathAndQuery);
